...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        return httpSecurity.sessionManagement(session ->
                ...
                // (서버사이드(SSR) 렌더링 하지 않는다. 즉 html화면을 백엔드가 만들지 않는다.)
                // 백엔드에서 화면을 만들지 않더라도 위 세팅을 끄지 않아도 괜찮다. 사용하지 않는 걸 끔으로써 리소스 확보
                // 하기 위해서 사용하는 개념
                // 정리하면, 시큐리티에서 제공해주는 로그인 화면 사용하지 않겠다.
                .formLogin(form -> form.disable()) //form 로그인 방식을 사용하지 않음을 세팅(예쁘지가 않음)
                ...
                .authorizeHttpRequests(auth ->
                        auth.requestMatchers(
                            ...
                            .anyRequest().authenticated()//로그인이 되어 있어야만 허용(위의 주소에 나와 있는 주소를 제외한)
                )
