...
/*
    ...
    인증받고 싶은 소셜로그인선택(redirect_uri 는 소셜 로그인 마무리되고 프론트엔드로 가는 url)
    소셜 로그인 화면 출력
    아이디/비번 입력해서 로그인 시도 (redirect_uri 는 백엔드로 가는 url) (요청정보 쿠키 저장)
    인가코드 받기 위한 작업이 이루어짐
    제공자(provider)는 아이디 / 비번이 일치한다면 백엔드 redirect_uri 로 인가코드를 보내준다.
    백엔드는 인가코드로 access_token 으로 사용자 정보를 받기위한 작업이 이루어짐
    로컬 로그인 작업 수행(SuccessHandler) (프론트엔드 redirect_uri 가 허용한 uri 인지 체크)
    프론트엔드 redirect_uri 로 리다이렉트를 하면서 필요한 정보를 파라미터로 보내준다.

    인증박고 싶은 소셜로그인 선택시 프론트엔드 redirect_uri 가 허용한 uri 인지 체크를 하도록 변경, 필터로 해결
 */
